...
select
       ...
       qt.user_id                                                                             as integration_user_id,
...
from {{ source('int_service_supply', 'cnc_order_quotes') }}  as quotes
       ...
       left join int_service_supply.quicklinks_tracking qt on qt.order_uuid = orders.uuid
       where (is_external='true' or qt.order_uuid is not null)
