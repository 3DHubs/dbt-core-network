...
disputes as (
    ...
    where not exists(select 1 from disputes_temp where disputes_temp.order_uuid = supply_order_history_events.order_uuid)
),

freshdesk_ticket_remake_request as (
...
--todo-migration-test -- This model was refactored because of a nested with and ambiguous dispute name