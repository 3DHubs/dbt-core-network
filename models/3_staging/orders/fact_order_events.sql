...
select
    *,
    case
        ...
        when
            description ~ 'changed order status to "shipped" on package' then 112
        ...
        when
            description ~ 'moved order to "shipped_to_warehouse"(.*on|\.)' then 114
        -- Excluding shipping updates from Shippo (see shipping_labels.py)
        ...
        when
            description ~ 'created a new purchase order to reimburse supplier' then 922
        when
            description ~ 'voided original purchase order to reimburse supplier' then 923
