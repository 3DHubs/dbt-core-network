with status as (SELECT order_uuid, is_cross_docking, status, time_line::date as start_status_date,
        coalesce(lag(start_status_date, 1) over (partition by order_uuid order by start_status_date desc), case when status not in ('completed_at','cancelled_at') then getdate()::date end) as end_status_date, date_diff('day',start_status_date,end_status_date) as verschil
FROM (SELECT order_uuid, is_cross_docking, created_at, submitted_at, closed_at, sourced_at,order_shipped_at, delivered_to_cross_dock_at, shipped_from_cross_dock_at, delivered_at, dispute_created_at, completed_at, cancelled_at FROM dbt_prod_reporting.fact_orders
...
select s.*,
       case  when s.status = 'created_at' then 'created'
             ...
             when s.status = 'dispute_created_at' then 'disputed' end as status_mapped,

       d.date from status s
