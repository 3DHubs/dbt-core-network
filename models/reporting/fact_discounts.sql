...
select so.uuid                                                  as order_uuid,
       ...
       l.discount_id,
       d.discount_factor,
       trunc(d.discount_value*1.0 / 100.00,2)                   as dicount_amount_local_currency,
       trunc((dicount_amount_local_currency / rates.rate), 2)   as discount_amount_usd,
       d.title,
       dc.description,
       dc.code,
...
from {{ ref('line_items') }} l
         ...
         inner join {{ ref('discount_codes') }} dc on dc.id = l.discount_code_id
         left join  {{ ref('users') }} u on u.user_id = dc.author_id
         left join  {{ source('data_lake', 'exchange_rate_spot_daily') }} as rates
                   on rates.currency_code_to = d.currency_code and
                      trunc(coalesce(coq.finalized_at, l.created)) = trunc(rates.date)
