select distinct fact_orders.hubspot_company_id,
       sum(case
               when fact_orders.is_closed and agg_orders.is_new_customer_company then order_closed_sales_usd
               end)
       over ( partition by fact_orders.hubspot_company_id) as new_customer_order_closed_sales_usd,
       sum(case
               when fact_orders.is_closed and agg_orders.is_new_customer_company then (fact_orders.order_sourced_sales_usd - fact_orders.sourced_cost_usd)
               end)
