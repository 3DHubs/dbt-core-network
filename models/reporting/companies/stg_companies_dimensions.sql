select distinct hubspot_company_id,
                ...
                first_value(market)
                over ( -- selects the country from the oldest contact of the company that has a country available
                    partition by hubspot_company_id order by (country_iso2 is null)::int, created_date asc rows between unbounded preceding and unbounded following) as market,
                first_value(continent)
                over ( -- selects the country from the oldest contact of the company that has a country available
                    partition by hubspot_company_id order by (country_iso2 is null)::int, created_date asc rows between unbounded preceding and unbounded following) as continent,
                first_value(region)
                over ( -- selects the country from the oldest contact of the company that has a country available
                    partition by hubspot_company_id order by (country_iso2 is null)::int, created_date asc rows between unbounded preceding and unbounded following) as region,
