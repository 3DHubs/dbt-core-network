...
with stg_agg_contact as (
    select distinct reporting_cube_deals.hubspot_contact_id as hubspot_contact_id,
    ...
    from {{ source('reporting', 'cube_deals') }} as reporting_cube_deals
     ...
     agg_contact_w_company as (
         select hubspot_contact_id,
                case
                    when not Bool_or(first_contact_deal_is_legacy) and
                ...
                case
                    when min(closed_date) > min(became_customer_date)
     ...
     stg_clients as (
         select distinct reporting_cube_deals.client_id      as client_id,
                         min(
...
select stg_clients."created_date",
       ...
       coalesce(stg_agg_contact.first_quote_country_iso2, stg_clients.country_iso2) as country_iso2,
