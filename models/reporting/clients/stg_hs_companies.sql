...
select hc.createdate                                                             as created_date,
       ...
       lower(nullif(coalesce(hca.country_iso2, ac2.alpha2_code, country_names.alpha2_code), '')) as country_iso2,
...
from {{ source('data_lake', 'hubspot_companies') }} hc
         ...
         left outer join {{ ref('industries') }} indm on indm.industry = lower(hc.industry)
         left outer join {{ ref('countries') }} as ac2
             on case when len(hc.country) = 2 then lower(hc.country) end = lower(ac2.alpha2_code)
         left outer join {{ ref('countries') }} as country_names
             on case when len(hc.country) >= 3 then lower(hc.country) end = lower(country_names."name")
