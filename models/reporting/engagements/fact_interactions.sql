...
with list_of_emails as (
    ...
    from list_of_emails emails
         ...
         left join (
             ...
             from {{ ref('freshdesk_agents') }} where _is_latest
    ...
    select he.id                                                                as interaction_id,
           ...
           coalesce(he.contact_id::varchar, cd.hubspot_contact_id::varchar)     as contact_id,
           coalesce(he.company_id::varchar, cd.hubspot_company_id::varchar)     as company_id,
           coalesce(dc.client_id, dcom.client_id, dcon.client_id, cd.client_id) as client_id,
           cd.order_uuid::varchar                                               as order_uuid,
           null::float                                                          as freshdesk_ticket_id
    
    from {{ ref('fact_hubspot_engagements') }} as he
            left join agents a on he.engagement_owner_id = a.hs_agent_id
            left join {{ source('reporting', 'cube_deals') }} as cd on he.deal_id = cd.hubspot_deal_id
            left join (
                select client_id, hs_company_id, hs_contact_id
                from {{ ref('dim_clients' ) }}
                where hs_company_id is not null
                    and hs_contact_id is not null
                ) as dc
                    on he.company_id = dc.hs_company_id and he.contact_id = dc.hs_contact_id
            left join (
                select client_id, hs_company_id
                from {{ ref('dim_clients' ) }}
                where hs_contact_id is null
                ) as dcom
                    on he.company_id = dcom.hs_company_id
            left join (
                select client_id, hs_contact_id
                from {{ ref('dim_clients' ) }}
                where hs_company_id is null
                ) as dcon
                    on he.contact_id = dcon.hs_contact_id
    ...
    select fdi.interaction_id                                 as interaction_id,
       ...
       case
           when fdi.interaction_type = 'agent reply' or fdi.interaction_type = 'agent initiation' then 'Outgoing Email'
       ...
       fd.hubspot_contact_id::varchar                         as contact_id,
       fd.hubspot_company_id::varchar                         as company_id,
       dc.client_id                                           as client_id,
       fd.order_uuid::varchar                                 as order_uuid,
       fdi.ticket_id                                          as freshdesk_ticket_id

    from {{ ref('fact_freshdesk_interactions') }} as fdi
            left join {{ ref('fact_freshdesk_tickets') }} fdt on fdi.ticket_id = fdt.ticket_id
            left join {{ source('reporting', 'cube_deals') }} fd on fd.order_uuid = fdt.order_uuid
            left join agents a on fdi.agent_id = a.fd_agent_id
            left join {{ ref('dim_clients') }} dc on dc.client_id = fd.client_id
