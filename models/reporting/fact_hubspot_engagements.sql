...
with stg_associations as (
    ...
    from {{ source('data_lake', 'hubspot_engagements') }} as he
...
from {{ source('data_lake', 'hubspot_engagements') }} as he
         ...
         left join {{ source('data_lake', 'hubspot_owners') }} ho on he.owner_id = ho.owner_id and ho.is_current is true
         left join {{ ref('hubspot_engagement_dispositions') }} hedm on he.disposition = hedm.uuid
