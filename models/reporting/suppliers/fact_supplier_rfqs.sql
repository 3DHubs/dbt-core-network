with winning_bid as (
    ...
    --query all supplier auctions that were RFQ'ed. One order can have multiple auctions per suppliers
    supplier_rfq_auctions as (
        ...
        from {{ ref('supplier_auctions') }} as sa
                inner join {{ ref('supplier_rfqs_src_auction') }} sr on sr.order_quotes_uuid = sa.auction_uuid
    ),
