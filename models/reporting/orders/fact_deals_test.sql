...
case when hubspot_contact_id then fd.became_customer_date_contact end as became_customer_date_contact,
case when hubspot_contact_id is not null then date_trunc('day',fd.became_opportunity_date_contact) end as became_opportunity_date_contact,
-- fd.cancellation_reason, -- Definition was changed, not comparable
fd.cancelled_at,
fd.change_request_freshdesk_ticket_id,
fd.change_request_status,
fd.city,
fd.closed_date,
case when hubspot_contact_id is not null then fd.closed_deal_days_between_previous_deal_contact end as closed_deal_days_between_previous_deal_contact,
-- case when hubspot_contact_id is not null then fd.closed_deal_days_between_previous_deal_from_delivery_contact end as closed_deal_days_between_previous_deal_from_delivery_contact, -- Old KPI
case when hubspot_contact_id is not null then fd.closed_deal_number_contact end as closed_deal_number_contact,
...
case when hubspot_company_id is not null then fd.first_bdr_owner_date end as first_bdr_owner_date,
...
fd.is_resourced,
fd.is_rfq,
case when ship_date_promised_by_supplier >= '2019-10-01' then fd.is_shipped_on_time_by_mp else null end as is_shipped_on_time_by_mp, 
--we  only show ship_date_promised_by_supplier >= '2019-10-01' in sp_cube_deals. but we define is_shipped_on_time_by_mp (in sp_stg_cube_deals) we use the po_shipping_date that includes deals < '2019-10-01'
...
-- fd.order_active_po_company_entity, -- We accepted the difference, DBT coming from DMS had more data
...
case when ship_date_promised_by_supplier >= '2019-10-01' then fd.shipping_by_supplier_days_delay else null end as shipping_by_supplier_days_delay, 
...
where true
and is_legacy_order is not true
