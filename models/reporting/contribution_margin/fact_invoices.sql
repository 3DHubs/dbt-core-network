...
stg_cube_invoices_supply as (
    ...
    from {{ ref('cnc_order_quotes') }} oqsl
                left outer join {{ ref('stg_fact_orders') }} fd using (order_uuid) -- This should be updated to the future `fact_deals` model.
...
stg_cube_invoices_netsuite as (
    ...
    from {{ ref('netsuite_invoices') }} as netsuite_trn
                ...
                left outer join {{ ref('stg_fact_orders') }} fd on fd.order_uuid = oqsl2.order_uuid -- To do: this dependency has to be changed once new fact_deals model is ready.
