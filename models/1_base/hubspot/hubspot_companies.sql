...
select hc.property_createdate__value                                                 as created_at,
       hc.property_name__value                                                       as name,
       (case
            when nullif(hc.property_numberofemployees__value__double, '') > 23000000 then null
            else hc.property_numberofemployees__value__double end)::int              as number_of_employees,
       ...
       (TIMESTAMP 'epoch' + hc.property_attempted_to_contact_date_company__value / 1000 *
                            INTERVAL '1 second')::timestamp without time zone        as attempted_to_contact_at,
       (TIMESTAMP 'epoch' + property_connected_date_company__value / 1000 *
                            INTERVAL '1 second')::timestamp without time zone        as connected_at,
       ...
       case
           when len(property_founded_year__value) < 5
               then nullif(nullif(property_founded_year__value, ''), 'N/A')::int end as founded_year,
       ...
       case
           ...
           when hc.property_deactivated__value = 'false'
               then false end ::boolean                                              as is_deactivated,
       (TIMESTAMP 'epoch' + hc.property_deactivated_date__value / 1000 *
                            INTERVAL '1 second')::timestamp without time zone        as deactivated_date,
       case
           when hc.property_reactivated_opportunity__value = 'true' then true
           when hc.property_reactivated_opportunity__value = 'false'
               then false end ::boolean                                              as is_reactivated_opportunity,
       (TIMESTAMP 'epoch' + hc.property_reactivated_opportunity_date__value / 1000 *
                            INTERVAL '1 second')::timestamp without time zone        as reactivated_opportunity_date,
       case
           when hc.property_reactivated_customer__value = 'true' then true
           when hc.property_reactivated_customer__value = 'false'
               then false end ::boolean                                              as is_reactivated_customer,
       (TIMESTAMP 'epoch' + hc.property_reactivated_customer_date__value / 1000 *
                            INTERVAL '1 second')::timestamp without time zone        as reactivated_customer_date,
       hc.property_company_lead_score__value::int                                    as lead_score,
       nullif(hc.property_tier__value__double, '')::int                              as tier,
       case
           when hc.property_qualified__value = 'true' then true
           when hc.property_qualified__value = 'false'
               then false end ::boolean                                              as is_qualified,
       case
           when hc.property_strategic__value = 'true' then true
